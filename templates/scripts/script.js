...
function getIntegration() {
    var inp = document.getElementById("function").value;
    fetch(" http://127.0.0.1:5000/", {
        method: "POST",
        "content-type":"application/json",
        body: JSON.stringify({input:inp}),
    }).then((res)=> res.json()).then((res)=>{
            document.getElementById("result").innerHTML = res.r1 + " + <span>&Sigma;<span> [" + res.r2 +"+"+ res.r3 + "]";
            expr = document.getElementById("function").value.toString();
            console.log(res.r1 + " + <span>&Sigma;<span> [" + res.r2 +"+"+ res.r3 + "]")
            expr = expr.replace("sin", "Math.sin").replace("cos", "Math.cos").replace("tan", "Math.tan").replace("exp", "Math.exp");
            sineArr.splice(0,sineArr.length);
            cosineArr.splice(0,cosineArr.length);
            inner(100);
            error(100);
        }
        )
